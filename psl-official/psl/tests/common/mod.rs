...
use saola_psl::{Configuration, SourceFile, parse_configuration_multi_file};
...
pub(crate) fn parse_unwrap_err(schema: &str) -> String {
    saola_psl::parse_schema_without_extensions(schema)
        .map(drop)
        .unwrap_err()
}

#[track_caller]
pub(crate) fn parse_schema(datamodel_string: &str) -> saola_psl::ValidatedSchema {
    saola_psl::parse_schema_without_extensions(datamodel_string).unwrap()
}

pub(crate) fn parse_config(schema: &str) -> Result<Configuration, String> {
    saola_psl::parse_configuration(schema).map_err(|err| err.to_pretty_string("schema.prisma", schema))
}

#[track_caller]
pub(crate) fn parse_configuration(datamodel_string: &str) -> Configuration {
    match saola_psl::parse_configuration(datamodel_string) {
        Ok(c) => c,
        Err(errs) => {
            panic!(
...
pub(crate) fn render_datasources(datamodel_string: &str) -> String {
    ...
    match parse_configuration_multi_file(&[("schema.prisma".into(), src)]) {
        Ok((files, config)) => saola_psl::get_config::render_sources_to_json(&config.datasources, &files),
...
pub(crate) fn expect_error(schema: &str, expectation: &expect_test::Expect) {
    match saola_psl::parse_schema_without_extensions(schema) {
