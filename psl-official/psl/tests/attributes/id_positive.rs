...
fn mapped_singular_id_must_work() {
    ...
    let datamodel = saola_psl::parse_schema_without_extensions(with_header(dml, Provider::Postgres, &[])).unwrap();
...
fn id_accepts_length_arg_on_mysql() {
    ...
    let schema = saola_psl::parse_schema_without_extensions(with_header(dml, Provider::Mysql, &[])).unwrap();
...
fn id_accepts_sort_arg_on_sqlserver() {
    ...
    let schema = saola_psl::parse_schema_without_extensions(with_header(dml, Provider::SqlServer, &[])).unwrap();
