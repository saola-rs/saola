...
async fn main() -> anyhow::Result<()> {
    ...
    let post1 = post()
        .create(
            format!("Rust ORM is awesome - {}", unique_ts),
    ...
    let post2 = post()
        .create(
            format!("Native Engines in Rust - {}", unique_ts),
            |u| { u.connect(|w| { w.id(created_user.id.clone()); }); }
        )
        .exec(&client)
        .await?;
    ...
    let created_users_count = user()
        .create_many()
        .data(|d| { d.email(format!("bulk-1-{}@example.com", unique_ts)); })
        .data(|d| { d.email(format!("bulk-2-{}@example.com", unique_ts)); })
        .exec(&client)
        .await?;
    ...
    let created_users_with_data = user()
        ...
        .data(|d| { d.email(format!("bulk-ret1-{}@example.com", unique_ts)); })
        .data(|d| { d.email(format!("bulk-ret2-{}@example.com", unique_ts)); })
        .exec(&client)
        .await?;
    println!(
    ...
    let updated_users = user()
        ...
        .where_clause(|w| { w.email().contains("bulk-ret".to_string()); })
        .data(|d| { d.display_name("Updated Bulk User".to_string()); })
        .exec(&client)
        .await?;
